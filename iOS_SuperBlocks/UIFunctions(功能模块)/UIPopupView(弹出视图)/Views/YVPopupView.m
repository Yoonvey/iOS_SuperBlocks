...
@interface YVPopupView()

@property (nonatomic, strong, readwrite) UIControl *blackOverlay;
@property (nonatomic, weak) UIView *containerView;
@property (nonatomic, assign, readwrite) YVPopupDirection popoverPosition;
...
@implementation YVPopupView{
    BOOL _isiOS7;
    BOOL _setNeedsReset;
}

+ (instancetype)popupView {
    return [[YVPopupView alloc] init];
}
...
- (void)commonInit {
    ...
    self.sideEdge = 10.0;
    self.maskType = YVPopuMaskTypeBlack;
...
- (void)showPopupViewWithPoint:(CGPoint)point
    ...
    switch (self.maskType) {
        case YVPopuMaskTypeBlack:
            maskColor = [UIColor colorWithWhite:0.0 alpha:0.3];
            break;
        case YVPopuMaskTypeNone: {
    ...
    self.popoverPosition = popupDirection;
...
- (void)showAtView:(UIView *)atView
    popoverPostion:(YVPopupDirection)position
   withContentView:(UIView *)contentView
    ...
    YVPopupDirection popoverPosition;
    if (upCanContain) {
        popoverPosition = YVPopupDirectionUp;
        atPoint.y = CGRectGetMinY(atViewFrame) - betweenArrowAndAtView;
    } else {
        popoverPosition = YVPopupDirectionDown;
    ...
    if (upCanContain && downCanContain) {
        ...
        if (position != 0) {
            useUp = position == YVPopupDirectionUp ? YES : NO;
        }
        if (useUp) {
            popoverPosition = YVPopupDirectionUp;
            atPoint.y = CGRectGetMinY(atViewFrame) - betweenArrowAndAtView;
    ...
    [self showPopupViewWithPoint:atPoint popupDirection:self.popoverPosition contentView:contentView inView:containerView];
}
...
- (void)show {
    ...
    CGFloat originY = 0.0;
    if (self.popoverPosition == YVPopupDirectionDown) {
